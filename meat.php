		...
		<div class="products_list"><!--procuct-list area-->
			<ul>
				<?php
					...
					global $db;
					$db = ierg4210_DB();
					$q = $db->prepare("SELECT * FROM products WHERE catid=1;");
					$product_array;
					if ($q->execute())
					...
					/* echo '$products["pid"]'.$products[0]["pid"]."\n";
					echo '$products["catid"]'.$products[0]["catid"]."\n";
					echo '$products["name"]'.$products[0]["name"]."\n";
					echo '$products["price"]'.$products[0]["price"]."\n";
					echo '$products["description"]'.$products[0]["description"]."\n"; */
					/* Underconstruction */
				?>
